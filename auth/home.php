...
require_once $_SERVER['DOCUMENT_ROOT'] . '/mas/config/fungsi.php';
?>
<style>
    #home {
        margin-bottom: 0;
...
<section id="churches" class="churches-section  py-5">
    <div class="container">
        <h2 class="text-center display-3 font-weight-bold"><?= webHome('judul') ?? ''; ?></h2>
    ...
    <div class="container">
        ...
        <div id="pdf-viewer-container">
            ...
            function getLatestPDF()
            {
                ...
                $url = "https://www.googleapis.com/drive/v3/files?q=\"{$folderId}\" in parents and mimeType='application/pdf'&orderBy=createdTime desc&key={$apiKey}";
                ...
                if (curl_errno($ch)) {
                    ...
                    return ['error' => 'Failed to fetch from Google Drive API'];
                }
                curl_close($ch);

                $data = json_decode($response, true);
                if (isset($data['files']) && count($data['files']) > 0) {
                    return ['url' => "https://drive.google.com/uc?id=" . $data['files'][0]['id']];
                } else {
                ...
                if (isset($data['files']) && count($data['files']) > 0) {
                    return "https://drive.google.com/uc?id=" . $data['files'][0]['id'];
                } else {
                    return null;
                }
            }
            ...
            <script type="module">
                ...
                async function fetchLatestPDF() {
                    const response = await fetch('/immanuel_community/fetch_latest_pdf.php');
                ...
                async function displayPDF() {
                    ...
                    try {
                        ...
                        viewer.innerHTML = ''; // Bersihkan viewer
        ...
        <div class="button-container">
            <button class="button button5">
                <a href="https://drive.google.com/drive/folders/1-yrd5aWTamIL8G0F3hIQxrueM6Kmt7j4?usp=sharing"
                    target="_blank" rel="noopener noreferrer" style="color:white; text-decoration: none;">Lainnya</a>
            </button>
        </div>
    </div>
</section>
